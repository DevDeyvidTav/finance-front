...
export default function FinancingsPage() {
  ...
  const createFinancing = useMutation({
    mutationFn: async (data: any) => {
      return api.post('/financings', data);
    },
    onSuccess: () => {
      setShowForm(false);
      setFormData({
  ...
  const financingTypes = ['vehicle', 'property', 'equipment', 'other'];
      ...
      <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  >
                    {financingTypes.map((type) => (
                      <option key={type} value={type}>
                        {type === 'vehicle' && 'Veículo'}
                        {type === 'property' && 'Imóvel'}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="submit"
                  disabled={createFinancing.isPending}
                  className="px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50"
                >
                  {createFinancing.isPending ? 'Salvando...' : 'Salvar Financiamento'}
                </button>
                <button
                  type="button"
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <p className="text-gray-600">
            Cadastre seus financiamentos de veículos, imóveis e outros bens. 
            O sistema calculará automaticamente o valor das parcelas mensais com base na taxa de juros.
          </p>
        </div>
      </div>
    </DashboardLayout>
